...
# TODO move to numpy_metrics
def get_binary_metrics(logits, labels, loss, return_all=False, thresh=0.5, name=""):
    logits = logits.reshape(-1, 1)
    # Replace torch.nn.functional.sigmoid with numpy equivalent
    probs = 1 / (1 + np.exp(-logits))
    pred = (probs > thresh).astype(np.float32)
    labels = labels.reshape(-1, 1)
    bin_metrics = get_classification_metrics(
        predicted=pred, labels=labels, n_classes=2)
    acc, precision, recall, F1, IOU = bin_metrics['micro']
    # Convert loss to float (assuming it's already a numpy scalar or can be converted)
    loss_ = float(loss)
    if return_all:
        return bin_metrics
